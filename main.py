...
intents.guilds = True
intents.message_content = True  # Important for readingsending messages
...
def webhook()
    ...
    try
        ...
        for tx in data.get(transactions, [])
            print(fProcessing transaction {tx})  # Debug print each transaction
            for event in tx.get(events, {}).get(tokenTransfers, [])
                print(fProcessing token transfer event {event})  # Debug print each token transfer event
                if event.get(tokenAddress) == TOKEN_ADDRESS
                    ...
                    tx_link = fhttpssolscan.iotx{tx['signature']}
                    msg = (
                        f🚀 {amount.2f} $YOURCOIN bought by `{buyer[4]}...{buyer[-4]}`n
